...
// append API call results to page 
...
//*Format the input by replacing spaces with
  ...
  if (searchValue !== null) {
  searchValue = searchValue.location(' ', '+');
    localStorage.setItem("location", searchValue);
     };
      
     var apiKey = "37ce3ed4c93e7e1b0d98d108a56559c4"
     var apitoday = ("https://api.openweathermap.org/data/2.5/weather?q=" + searchValue + "&units=imperial" + "&appid=" + apiKey)
     console.log(apitoday);
      ...
      //forecast 
      var apiForecast = ("https://api.openweathermap.org/data/2.5/forecast?q=" + searchValue + "&units=imperial&appid=" + apiKey);
  ...
  function getCity() {
   ...
   var result = getCity.searchValue[getCity.selectedIndex].text;
   
  

  
  
  // event handler that responds when user enters text and clicks search
  ...
  const apikey= "37ce3ed4c93e7e1b0d98d108a56559c4";
...
// clears local storage
  ...
  // Ajax Call to API
    ...
    function searchWeather(searchValue) {
      $.ajax({
        type: "GET",
        url:
        "http://api.openweathermap.org/data/2.5/forecast?id=524901&APPID=37ce3ed4c93e7e1b0d98d108a56559c4"}) 
      } 
  ...
  //API call to pull data for today
  ...
  // Create Materialize cards to display forecast results 
  
  
  // Append Materialize cards to display forecast results 
